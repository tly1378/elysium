...
def main():
    while True:
        ...
        while True:
            print(
                f"{Colors.BLUE}Current dialogues: {','.join(map(str, current))}{Colors.RESET}")
            id = input(">>> ")
            try:
                id = int(id)
                if id in current:
        ...
        while True:
            ...
            if cfg is None:
                ...
                break
            print(
                f"{Colors.BOLD}{cfg.group_id}-{cfg.id} {cfg.actor}: {Colors.RESET}{cfg.text}", end='')
            ...
            if cfg.selections != None and len(cfg.selections) > 0:
                ...
                for dialogue_id in cfg.selections:
                    dialogue = tables.TbDialogue.get(
                        dialogue_id.group_id, dialogue_id.id)
                    if dialogue.item_requirements is None or all(item in items for item in dialogue.item_requirements): # type: ignore

...
namespace SurvivalEngine
{
    ...
    public class ItemSlotPanel : UISlotPanel
    {
        ...
        protected int inventory_owner = -1;
        ...
        public virtual void InitPanel()
        {
            if (!IsPlayerSet())
            {
                ...
                PlayerCharacter player = player_ui ? player_ui.GetPlayer() : PlayerCharacter.GetSelf();
        ...
        protected override void RefreshPanel()
        {
            ...
            inventory.limited_slot = limit_one_item;
        ...
        private void OnClick(UISlot uislot)
        {
            if (uislot != null)
            {
                ...
                else if (islot.GetCraftable() != null)
                {
                    ...
                    AAction aaction = idata?.FindAutoAction(player, islot.GetSlot());
                    if (player != null && aaction != null)
                        player.DoClickAction(aaction, islot.GetSlot());
        ...
        private void OnDragEnd(UISlot aslot)
        {
            ...
            if (slot != null && slot.GetItem() != null)
            {
                ...
                if (player != null && maction != null 
                    ...
                    && maction.CanDoAction(player, slot.GetSlot(), select))
                {
                    player.DoAction(maction, slot.GetSlot(), select);
                }
            }
        }
        ...
        public void SetInventory(InventoryType type, string uid, int owner, int size)
        {
            inventory_type = type;
            inventory_uid = uid;
            ...
            InventoryData idata = InventoryData.Get(type, uid, owner);
            if(idata != null)
                idata.size = size;
        }
        ...
        {
            return current_player ? current_player.PlayerID : 0;
        }
        ...
        public void MergeSlots(ItemSlot selected_slot, ItemSlot clicked_slot)
        {
            if (selected_slot != null && clicked_slot != null && current_player != null)
            {
                ...
                if (item1 != null && item2 != null && item1 != item2)
                {
                    ...
                    //Use merge action
                    if (action1 != null && action1.CanDoAction(current_player, slot1.GetSlot(), slot2.GetSlot()))
                    {
                        DoMergeAction(action1, slot1, slot2);
                        return;
                    }

                    else if (action2 != null && action2.CanDoAction(current_player, slot2.GetSlot(), slot1.GetSlot()))
                    {
        ...
        private void DoMergeAction(MAction action, ItemSlot slot_action, ItemSlot slot_other)
        {
            ...
            current_player.DoAction(action, slot_action.GetSlot(), slot_other.GetSlot());
        ...
        public void MoveItem(ItemSlot slot1, ItemSlot slot2)
        {
            ...
            current_player.Inventory.MoveItem(slot1.GetSlot(), slot2.GetSlot());
            CancelPlayerSelection();
        }
        ...
        public void CancelPlayerSelection()
        {
            ...
            {
                PlayerUI player_ui = PlayerUI.Get(current_player.PlayerID);
                if (player_ui != null)
        ...
        {
            return InventoryData.Get(inventory_type, inventory_uid, inventory_owner);
        }

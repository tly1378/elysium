...
namespace SurvivalEngine
{
    ...
    public class ItemData : CraftData
    {
        ...
        public AAction FindAutoAction(PlayerCharacter character, InventorySlot islot)
        {
            foreach (SAction action in actions)
            {
        ...
        public static new void Load(string folder = "")
        {
            if (!loaded)
            {
                ...
                foreach (ItemData data in item_data)
                {
                    TheNetwork.Get().RegisterPrefab(data.item_prefab);
                    TheNetwork.Get().RegisterPrefab(data.equipped_prefab);
                    TheNetwork.Get().RegisterPrefab(data.projectile_prefab);
                }
            }
        }

...
namespace NetcodePlus
{
    ...
    public class TheNetwork : MonoBehaviour
    {
        ...
        {
            if (NetworkData.Get().solo_type == SoloType.Offline)
                StartHostOffline();
            else
                StartHostOnline(NetworkData.Get().game_port);
        }
        ...
        public void StartHostOnline(ushort port)
        {
        ...
        {
            foreach (GameObject prefab in NetworkData.Get().network_prefabs)
            {
        ...
        public static TheNetwork GetIfExist()
        {
            return instance;
        }

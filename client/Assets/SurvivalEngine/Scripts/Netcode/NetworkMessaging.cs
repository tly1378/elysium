...
namespace NetcodePlus
{
    ...
    public class NetworkMessaging
    {
        ...
        {
            Send(type, ClientReadyList, writer, delivery);
        }
        ...
        public void ForwardAll(string type, ulong source_client, FastBufferReader reader, NetworkDelivery delivery)
        {
            if (IsServer && IsOnline)
            {
                ...
                foreach (ulong client in ClientReadyList)
                {
        ...
        public IReadOnlyList<ulong> ClientReadyList { get { return network.GetReadyClients(); } }
        public int ClientCount { get { return network.GetClientsIds().Count; } }

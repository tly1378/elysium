...
namespace SurvivalEngine
{
    ...
    public class PlayerCharacter : SNetworkBehaviour
    {
        ...
        protected override void OnSpawn()
        {
            ...
            actions.RegisterSerializable(ActionType.Click, DoClick);
            actions.RegisterSerializable(ActionType.ClickRight, DoRightClick);
        ...
        private void OnClick(Vector3 pos, Selectable select) { actions.Trigger(ActionType.Click, new NetworkActionClickData(select, pos)); } // DoClick(pos)
        private void OnRightClick(Vector3 pos, Selectable select) { actions.Trigger(ActionType.ClickRight, new NetworkActionClickData(select, pos)); } // DoRightClick(pos)
        ...
        private void FixedUpdateNotOwner()
        {
            ...
            if (offset.magnitude > moving_threshold)
                rigid.linearVelocity = moving + offset;
            else
                rigid.linearVelocity = moving;
        ...
        {
            if (render != null && IsSelf())
            {
                Vector3 campos = TheCamera.Get().transform.position;
                float dist = (campos - transform.position).magnitude;
                bool should_show = dist >= 4f; //character disappear when camera is too close
                if (render.enabled != should_show)
                    render.enabled = should_show;
            }
        }
        ...
        private Vector3 FindMovementDirection()
        {
            ...
            if (IsClimbing())
                return character_climb.GetMove();
        ...
        private void DoClick(SerializedData sdata)
        {
            NetworkActionClickData ndata = sdata.Get<NetworkActionClickData>();
        ...
        private void DoRightClick(SerializedData sdata)
        {
            //NetworkActionClickData ndata = sdata.Get<NetworkActionClickData>();
            //Selectable selectable = ndata.GetSelectable();
            //Vector3 pos = ndata.pos;

...
namespace SurvivalEngine
{
    ...
    public class PlayerCharacterAnim : MonoBehaviour
    {
        ...
        private AnimatorControllerParameter[] anim_params;

        void Awake()
        {
            character = GetComponent<PlayerCharacter>();
            animator = GetComponentInChildren<Animator>();
            anim_params = animator.parameters;
        ...
        public void TriggerAnim(string id, float duration)
        {
            SetAnimTrigger(id);
            character.onTriggerAnim?.Invoke(id, duration);
        }

        public void SetAnimBool(string id, bool value)
        {
            if (animator != null && HasAnimParam(id))
        ...
        private bool HasAnimParam(string anim_id)
        {
            ...
            foreach (AnimatorControllerParameter param in anim_params)
            {
        ...
        private void OnAttack(AttackTarget target, bool ranged)
        {
        ...
        private void OnAttackHit(AttackTarget target)
        {

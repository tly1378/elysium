...
namespace SurvivalEngine
{
    ...
    public abstract class MAction : SAction
    {
        ...
        public virtual void DoAction(PlayerCharacter character, InventorySlot slot, InventorySlot slot_other)
        {

        }

        //When using an ItemData action on a Selectable (ex: cook meat)
        public virtual void DoAction(PlayerCharacter character, InventorySlot slot, Selectable select)
        {

        }

        //Condition when merging item-item, override if you need to add a new condition
        public virtual bool CanDoAction(PlayerCharacter character, InventorySlot slot, InventorySlot slot_target) //slot_target is the one without the action
        {
        ...
        public virtual bool CanDoAction(PlayerCharacter character, InventorySlot slot, Selectable select)
        {

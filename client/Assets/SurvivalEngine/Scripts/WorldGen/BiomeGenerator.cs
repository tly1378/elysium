...
namespace SurvivalEngine.WorldGen
{
    ...
    public class BiomeGenerator : MonoBehaviour
    {
        ...
        void Start()
        {
            if (mode == WorldGeneratorMode.Runtime && Application.isPlaying && TheNetwork.Get().IsServer)
            {
        ...
        {
            if (!WorldData.Get().IsWorldGenerated())
            {
                GenerateRandomWorld();
            }
            else
            {
                GenerateRandomWorld(WorldData.Get().world_seed);
            }
        }
        ...
        {
            ClearBiomeObjects();
            SNetworkObject.editor_auto_gen_id = false; //Turn off OnValidate otherwise extremely slow
            ...
            SNetworkObject.editor_auto_gen_id = true;  //Turn back on
        }
        ...
        public void GenerateBiomeUID()
        {
            ...
            SNetworkObject[] all_nobj = GetComponentsInChildren<SNetworkObject>();
            SNetworkObject.GenerateAll(all_nobj);

            if (Application.isPlaying)
                SNetworkObject.SpawnAll(all_nobj);
        }

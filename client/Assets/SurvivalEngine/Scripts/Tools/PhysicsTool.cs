...
namespace SurvivalEngine
{
    ...
    {
        private static RaycastHit[] hits = new RaycastHit[10]; //Reused buffer for raycastall
        ...
        //Detect if object is grounded, the ground normal, and ground distance from root
        public static bool DetectGroundArea(Transform root, Vector3 center, float hdist, float max_dist, float radius, LayerMask ground_layer, out float ground_distance, out Vector3 ground_normal)
        {
            Vector3 p1 = center;
        ...
        public static void DetectGroundLayers(Vector3 center, float hdist, List<int> layers)
        {
            Vector3 p1 = center;
            int nb_hits = Physics.RaycastNonAlloc(p1, Vector3.down, hits, hdist, ~0, QueryTriggerInteraction.Ignore);

            layers.Clear();
            for (int i = 0; i < nb_hits; i++)
                layers.Add(hits[i].collider.gameObject.layer);
        }
        ...
        public static bool IsAnyLayerInLayerMask(List<int> layers, LayerMask mask)
        {

...
namespace SurvivalEngine
{
    ...
    public class Buildable : SNetworkBehaviour
    {
        [Header("Buildable")]
        ...
        void Update()
        {
            if (building_mode && building_character != null && building_character.IsSelf())
            {
                ...
                if (!position_set)
                {
                    if (controls.IsGamePad())
                    {
                        ...
                        transform.position = GetBuildFrontPos(building_character);
        ...
        public void StartBuild(PlayerCharacter character)
        {
            ...
            PlayerControls controls = PlayerControls.Get();
            PlayerControlsMouse mouse = PlayerControlsMouse.Get();
            transform.position = controls.IsGamePad() ? GetBuildFrontPos(building_character) : mouse.GetPointingPos();
        ...
        public void SetBuildPositionTemporary(Vector3 pos, Vector3 rot)
        {
            if (building_mode)
            {
                transform.position = pos;
        ...
        public bool CheckIfOverlap()
        {
            ...
            foreach (Collider overlap in overlap_colliders)
            {
                if (overlap != null)
                {
                    ...
                    PlayerCharacter player = overlap.GetComponentInParent<PlayerCharacter>();
        ...
        public Vector3 GetBuildFrontPos(PlayerCharacter character)
        {
            Vector3 forward = TheCamera.Get().IsFreelook() ? TheCamera.Get().GetFacingFront() : building_character.transform.forward;
            return character.transform.position + forward * GetBuildRange(character);
        }

        public bool IsInRange(PlayerCharacter character)
        {
            float dist = (transform.position - character.transform.position).magnitude;
            float brange = GetBuildRange(character);
            return dist <= brange;
        }

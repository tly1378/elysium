...
namespace SurvivalEngine
{
    ...
    public class ItemProvider : SNetworkBehaviour
    {
        [Header("Item Spawn")]
        ...
        {
            if (WorldData.Get().HasCustomInt(GetAmountUID()))
                nb_item = WorldData.Get().GetCustomInt(GetAmountUID());
        ...
        void Update()
        {
            ...
            if (item_progress > item_spawn_time)
            {
                ...
                WorldData.Get().SetCustomInt(GetAmountUID(), nb_item);
            }
        ...
        {
            TheAudio.Get().PlaySFX3D("item", take_sound, transform.position);
        }

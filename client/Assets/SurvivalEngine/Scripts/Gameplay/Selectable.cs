...
namespace SurvivalEngine
{
    ...
    public class Selectable : SNetworkBehaviour
    {
        ...
        private static LinkedList<Selectable> active_list = new LinkedList<Selectable>();
        private LinkedListNode<Selectable> active_node;

        protected override void Awake()
        {
            base.Awake();
            active_node = new LinkedListNode<Selectable>(this);
        ...
        public void Use(PlayerCharacter character, Vector3 pos)
        {
            if (enabled)
            {
                PlayerUI ui = PlayerUI.Get(character.PlayerID);
                ...
                if (maction != null && slot != null && maction.CanDoAction(character, slot.GetSlot(), this))
                {
                    character.DoAction(maction, slot.GetSlot(), this);
                    PlayerUI.Get(character.PlayerID)?.CancelSelection();
                }
                else if (aaction != null && aaction.CanDoAction(character, this))
                {
                    character.DoAction(aaction, this);
                }
        ...
        public int FindActionIndex(SAction action)
        {
            int index = 0;
            foreach (SAction aaction in actions)
            {
                if (aaction == action)
                    return index;
                index++;
        ...
        public SAction GetAction(int index)
        {
        ...
        {
            return IsActive() && type != SelectableType.CantSelect;
        }

        //Player can interact with it by clicking on it, such as using actions
        public bool CanBeInteracted()
        {
            return IsActive() && type != SelectableType.CantInteract && type != SelectableType.CantSelect;
        }
        ...
        public static Selectable GetNearestRaycast(float range=20f)
        {
            float min_dist = range;
            Selectable nearest = null;
            Vector3 pos = PlayerControlsMouse.Get().GetMouseWorldPosition();
            foreach (Selectable select in PlayerControlsMouse.Get().GetRaycastList())
            {
                Vector3 dist = (select.GetClosestInteractPoint(pos) - pos);
        ...
        public static Selectable GetNearestHover(Vector3 pos, float range = 999f)
        {
            ...
            foreach (Selectable select in active_list)
            {
                if (select.enabled && select.IsHovered())
                {
                    float dist = (select.transf.position - pos).magnitude;
                    if (dist < min_dist)
                    {
        ...
        public static LinkedList<Selectable> GetAllActive()
        {
            return active_list;
        }
    }

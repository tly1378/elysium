...
namespace SurvivalEngine
{
    ...
    public class Construction : Craftable
    {
        ...
        private static LinkedList<Construction> construct_list = new LinkedList<Construction>();
        private LinkedListNode<Construction> node;

        protected override void Awake()
        {
            base.Awake();
            node = construct_list.AddLast(this);
        ...
        private void OnBuild()
        {
            if (data != null)
            {
                BuiltConstructionData cdata = WorldData.Get().AddConstruction(data.id, SceneNav.GetCurrentScene(), transform.position, transform.rotation, data.durability);
                unique_id.unique_id = cdata.uid;
                NetObject.SetOptimizerEnabled(true);
                NetObject.Spawn();
            }
        }
        ...
        private void OnDeath()
        {
            ...
            WorldData.Get().RemoveConstruction(GetUID());
        ...
        {
            get { return WorldData.Get().GetConstructed(GetUID()); }  //Can be null if not built or spawned
        }
        ...
        public static LinkedList<Construction> GetAll()
        {
        ...
        {
            BuiltConstructionData bdata = WorldData.Get().GetConstructed(uid);
            if (bdata != null && bdata.scene == SceneNav.GetCurrentScene())
            {
                ...
                if (cdata != null)
                {
                    ...
                    construct.unique_id.was_created = true;
                    construct.unique_id.unique_id = uid;
                    construct.NetObject.Spawn();
                    return construct;
                }
            }
            return null;
        ...
        public static Construction CreateBuildMode(ConstructionData data, Vector3 pos)
        {
            ...
            construct.data = data;
            construct.unique_id.was_created = true;
            construct.NetObject.SetOptimizerEnabled(false); //Disable optimizer while in build mode
            return construct;
        }

...
namespace SurvivalEngine
{
    ...
    public class AnimalLivestock : SNetworkBehaviour
    {
        ...
        protected override void OnSpawn()
        {
            ...
            actions.Register(ActionType.Craft, DoProduceItem);
            actions.RegisterBehaviour(ActionType.Eat, DoStartEat);
        ...
        private void UpdateBehavior()
        {
            ...
            {
                if (target_food == null || state_timer > 5f)
                {
                    ChangeState(LivestockState.Wander);
                    return;
                }
        ...
        private void StartEat(AnimalFood food)
        {
            actions?.Trigger(ActionType.Eat, food);
        }

        private void DoStartEat(SNetworkBehaviour beha)
        {
            target_food = beha.Get<AnimalFood>();
            if (target_food == null)
                return;

            character.Stop();
            character.FaceTorward(target_food.transform.position);
            DoChangeState(LivestockState.Eat);
            if (animator != null)
                animator.SetTrigger("Eat");

            AnimalFood food = target_food;
            TimeTool.WaitFor(2f, () =>
            {
                if (food != null)
                {
        ...
        private void ProduceItem()
        {
            ...
            {
                actions?.Trigger(ActionType.Craft);
            }
        }

        private void DoProduceItem()
        {
            if (item_produce != null)
            {
                ResetGrowTime();
                ...
                {
                    int count_animal = Character.CountInRange(character.data, transform.position, item_max_range);
                    int count_item = Item.CountInRange(item_produce, transform.position, item_max_range);
        ...
        private void DoChangeState(LivestockState state)
        {
            this.state = state;
            state_timer = 0f;
        }

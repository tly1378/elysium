...
namespace SurvivalEngine
{
    ...
    public class TheAudio : MonoBehaviour
    {
        ...
        private void Start()
        {
            AudioListener.volume = SettingData.Get().master_volume;
        }
        ...
        public void PlaySFX(string channel, AudioClip sound, float vol = 0.8f, bool priority = true)
        {
            ...
            float volume = WorldData.Get() != null ? SettingData.Get().sfx_volume : 1f;
        ...
        //Play a sound only if the camera is in range of pos
        public void PlaySFX3D(string channel, AudioClip sound, Vector3 pos, float vol = 0.8f, float range_min = 10f, float range_max = 20f, bool priority = true)
        {
            if (string.IsNullOrEmpty(channel) || sound == null)
                return;

            Vector3 tpos = TheCamera.Get() != null ? TheCamera.Get().transform.position : Camera.main.transform.position;
            float dist = (tpos - pos).magnitude;
            float vol3 = 1f - Mathf.Clamp01((dist - range_min) / (range_max - range_min));
            if (vol3 > 0.02f)
                PlaySFX(channel, sound, vol * vol3, priority);
        }
        ...
        public void PlayMusic(string channel, AudioClip music, float vol = 0.4f, bool loop = true)
        {
            ...
            float volume = WorldData.Get() != null ? SettingData.Get().music_volume : 1f;
            channels_volume[channel] = vol;
        ...
        public void RefreshVolume()
        {
            ...
            foreach (KeyValuePair<string, AudioSource> pair in channels_sfx)
            {
                if (pair.Value != null)
                {
                    ...
                    pair.Value.volume = vol * SettingData.Get().sfx_volume;
                }
            }
            ...
            foreach (KeyValuePair<string, AudioSource> pair in channels_music)
            {
                if (pair.Value != null)
                {
                    ...
                    pair.Value.volume = vol * SettingData.Get().music_volume;
                }
            }
        }
